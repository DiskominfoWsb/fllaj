...
Class PaginateClass{
	function PaginateClass($nrw, $numperpage = 15, $numsetpg = 5, $linkformat = "?pg={pg}&pgs={pgs}"){	
			...
			$pages = ""; 
			if($pgs>'1'){
				$pages = $pages."<a href=\"".$this->getLink($linkformat,$x,$prev)."\"> <p class=\"transbg\"> &laquo; Prev </p> </a> ";
				} 
			...
			for($x=$awal;$x<=$akhir;$x++){			
				if($x<=$numpg){
					$addstyle = ($x==$pg)?"aktif":"";
					$pages = $pages."<a href=\"".$this->getLink($linkformat,$x,$pgs)."\"> <p class=\"$addstyle transbg\">$x</p> </a>";
				}
			}
			$next = $pgs+1;
			if($pgs<$lssetpg){
				$pages = $pages." <a href=\"".$this->getLink($linkformat,$x,$next)."\"> <p class=\"transbg\"> Next &raquo; </p> </a>";
			}
			$pages = $pages."</ul>";
	...
	function getLink($linkformat,$pg,$pgs){	
		$pg = ($pg=='')?'1':$pg;
		$linkformat =  ereg_replace('{pg}',"$pg",$linkformat);
		$linkformat =  ereg_replace('{pgs}',"$pgs",$linkformat);
		return $linkformat;
	}
}
